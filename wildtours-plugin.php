...
/**
 * Central plugin admin menu slug.
 *
 * All Panna Wild Tour admin pages and custom post types should use
 * this constant as their parent menu.
 */
define('PWT_ADMIN_MENU_SLUG', 'pwt-dashboard');
...
/**
 * Render unsupported environment notice.
 */
function pwt_render_environment_notice(): void
{
    ...
    echo '<div class="notice notice-error"><p>'
        . esc_html__(
            'Panna Wild Tour requires WordPress 6.7+ and PHP 8.2+. The plugin was not booted.',
            'wildtours-plugin'
        )
        . '</p></div>';
}
 ...
 *
 * Individual integrations may override this through the filter.
 */
add_filter(
    'pwt_verify_payment_webhook',
    static function (
        mixed $status,
        mixed $paymentId
    ): mixed {
 ...
 */
add_filter(
    'plugin_action_links_' . PWT_PLUGIN_BASENAME,
    static function (array $links): array {
        $custom = [
            '<a href="' . esc_url(
                admin_url(
                    'admin.php?page=pwt-settings'
                )
            ) . '">'
                . esc_html__(
                    'Settings',
            ...
            '<a href="' . esc_url(
                admin_url(
                    'edit.php?post_type=pwt_booking'
                )
            ) . '">'
                . esc_html__(
                    'Bookings',
